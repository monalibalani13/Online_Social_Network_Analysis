...
def bfs(graph, root, max_depth):
    ...
    for i in neighbors:
        if i == root:
            neighbors.extend(graph.neighbors(i))
            ...
            if count > max_depth:
                break
            for j in neighbors:
                if j not in node2distances:
                    node2distances[j] = count
    
        else:
            count = node2distances[i] + 1
            if count > max_depth:
                break
            for k in graph.neighbors(i):
                if k not in neighbors:
                    neighbors.append(k)
                if k not in node2distances:
                    node2distances[k] = count
            visited_nodes.append(i)
    ...
    for i in neighbors:
        neighbors_present = graph.neighbors(i)
        for j in neighbors_present:
            if j not in node2distances:
                continue
            if ((node2distances[j] > node2distances[i]) & (node2distances[j] <= max_depth)):
                node2parents[j].append(i)

    node2num_paths = defaultdict(int)
    traversed_nodes = deque()
    neighbors_path = deque([root])
    depth = 0
    l = []
    
    while depth <= max_depth:
        if len(traversed_nodes) == len(neighbors):
            break
        
        for i in neighbors_path:
            if depth == int(node2distances[i]):
                if i not in traversed_nodes:
                    traversed_nodes.append(i)
                node2num_paths[i] = node2num_paths[i] + 1
                l.extend(graph.neighbors(i))
        neighbors_path.clear()
        neighbors_path.extend(l)
        del l[:]
        depth = depth + 1
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    """
    n_credit = defaultdict(lambda:1)
    e_credit = {}
    
    for i in reversed(list(node2distances)):
        if i not in node2parents:
            break
        num_of_parents = len(node2parents[i])
        for j in node2parents[i]:
            if i< j:
                e_credit[(i,j)] = float(n_credit[i]/num_of_parents)
                n_credit[j] = n_credit[j] + e_credit[(i,j)]
            else:
                e_credit[(j,i)] = float(n_credit[i]/num_of_parents)
                n_credit[j] = n_credit[j] + e_credit[(j,i)]
...
def approximate_betweenness(graph, max_depth):
    ...
    for n in nodes:
        ...
        e_credit = bottom_up(n, node2distances, node2num_paths, node2parents)
        c.update(e_credit)
...
def partition_girvan_newman(graph, max_depth):
    ...
    betweenness = sorted(sorted(approximate_betweenness(graph2, max_depth).items()), key=lambda x: (-x[1],x[0]))
    
    edges = graph2.edges()
    
    for i in betweenness:
        num_of_components = nx.number_connected_components(graph2)
        if num_of_components == 1:
            if i[0] in edges:
                graph2.remove_edge(*i[0])
        else:
            break

    l1 = list(get_components(graph2))
...
def cut(S, T, graph):
    ...
    for s in S:
        for t in T:
            if (graph.has_edge(s,t)) | (graph.has_edge(t,s)):
...
def norm_cut(S, T, graph):
    ...
    """
    value_S = volume(S,graph)
    value_T = volume(T,graph)
    
    cut_value = cut(S, T, graph)
    
    normalized_cut_value = ((cut_value/value_S) + (cut_value/value_T))
...
def score_max_depths(graph, max_depths):
    ...
    for i in max_depths:
        ...
        nc_value = norm_cut(S, T, graph)
        score.append((i, nc_value))
...
def make_training_graph(graph, test_node, n):
    ...
    neighbor = graph2.neighbors(test_node)
...
def evaluate(predicted_edges, graph):
    ...
    """
    match_edges = 0
    no_of_edges = len(predicted_edges)
    
    for e in predicted_edges:
        if graph.has_edge(*e):
            match_edges = match_edges + 1
    
    evaluation = match_edges/no_of_edges
